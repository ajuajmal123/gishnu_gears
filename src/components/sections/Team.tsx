      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-20">
          {heads.map((head) => (
            <Card
              key={head.id}
              className="flex flex-col justify-between items-stretch text-left bg-white border border-slate-200 min-h-[460px] p-6 shadow-sm hover:border-brand-orange/20"
            >
              {/* Header Info */}
              <div className="flex flex-col gap-3">
                <div className="p-2.5 rounded bg-slate-50 border border-slate-100 text-brand-orange self-start flex items-center justify-center">
                  {head.icon}
                </div>
                <div>
                  <span className="text-[9px] font-mono text-brand-navy/40 uppercase tracking-widest block font-bold mb-1">
                    {head.dept}
                  </span>
                  <h4 className="text-base sm:text-lg font-sora font-extrabold text-brand-navy tracking-tight">
                    {head.name}
                  </h4>
                  <p className="text-brand-orange font-mono text-[10px] uppercase tracking-wider font-bold mt-0.5">
                    {head.role}
                  </p>
                </div>

                <p className="text-brand-navy/60 text-xs leading-relaxed font-sans mt-1.5 min-h-[72px]">
                  {head.biography}
                </p>
              </div>
              ...
              <div className="mt-6 pt-3 border-t border-slate-100 flex flex-col gap-3">
                {/* Metric */}
                <div className="flex justify-between items-center bg-slate-50/50 border border-slate-100 rounded px-3 py-2 text-left">
                ...
                <motion.a
                  ...
                  whileTap={{ scale: 0.99 }}
                  href={head.linkedin}
                  className="w-full inline-flex items-center justify-center gap-2 px-3 py-2.5 rounded bg-brand-navy hover:bg-brand-navy-light text-white font-semibold text-[10px] tracking-wider uppercase transition-all duration-200 shadow-sm cursor-pointer"
                >
